...
| `--appearance <mode>` | `auto`, `light`, or `dark`. See below |
...
PAIR picks its colours from your terminal's background, which it detects by
asking the terminal and waiting for a reply. Some terminals do not answer —
this is common over SSH and inside `tmux` — and PAIR then assumes a dark
background. On a light theme the result is washed-out text and highlights you
cannot read through.
